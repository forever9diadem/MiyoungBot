...
helpmessage = "Hi I'm MiyoungBot v"+version+"! I was created by forever9diadem\n\
...
thumbsup = ['https://57.media.tumblr.com/576937e2dc3e53298df6b26a4ec38d47/tumblr_ny8b06hBuQ1ti35kvo6_400.gif']
...
####### MiyoungBotcommands #####
        if message.content[0:5].lower() == '!add ':
            ...
            if len(message_list) < 3:
                ...
                check = True
                t = open('MiyoungBotcommands.txt', 'r+')
                c = open('MiyoungBotcommandslist.txt', 'r+')
                try:
...
################
        if message.content[0:8].lower() == '&&delete' or message.content[0:7].lower() == '!delete':
            if message.content[0:7].lower() == '!delete' and message.author.id != '68661361537712128':
                ...
                newt = '#MiyoungBot_commands#'
                for line in t:
                    if new[1] == line.split()[0]:
                        willdelete=1
                    elif line != '#MiyoungBot_commands#\n':
                        newt += '\n' + line.strip('\n')
...
################
        if message.content.split()[0].lower() == '$undo':
            try:
                t = open('tMiyoungBotcommands.txt', 'r+')
                c = open('MiyoungBotcommandslist.txt', 'r+')
                if can_undo == False:
                    ...
                    #t = open('MiyoungBotcommands.txt', 'r+')
                    #c = open('MiyoungBotcommandslist.txt', 'r+')
                    line_count = file_len('MiyoungBotcommands.txt')
...
################
        if message.content[0:7].lower() == '!random' and message.channel.id != '133389185988952064':
            t = open('MiyoungBotcommands.txt', 'r+')
            lines = file_len('MiyoungBotcommands.txt')
...
def handle_commands(message):
    for line in open('MiyoungBotcommands.txt', 'r+'):
